...
import { englishResources, type ResourceArticle } from "@/data/resource-content";
...
function Resource({ article }: { article: ResourceArticle }) {
  return <Shell eyebrow="RESOURCE · DRAFT" title={article.title} description={article.description}><Answers paragraphs={[article.directAnswer]} /><section className={sectionClass}><h2 className="text-[clamp(2rem,4vw,4rem)] font-light">Editorial outline</h2><div className="mt-[2rem] grid gap-[1rem]">{article.outline.map((section) => <section key={section.heading} className="border border-white/15 p-[1.5rem]"><h2 className="text-[1.5rem] font-light">{section.heading}</h2><p className="mt-[0.75rem] text-signal">{section.guidance}</p></section>)}</div></section><section className={sectionClass}><h2 className="text-[clamp(2rem,4vw,4rem)] font-light">Data table</h2><div className="mt-[2rem] overflow-x-auto"><table className="w-full min-w-[48rem] border-collapse"><caption className="mb-[1rem] text-left text-white/60">{article.table.caption}</caption><thead><tr>{article.table.columns.map((column) => <th key={column} className="border border-white/20 bg-white/10 p-[0.75rem] text-left">{column}</th>)}</tr></thead><tbody>{article.table.rows.map((row, index) => <tr key={`${article.path}-${index}`}>{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`} className="border border-white/20 p-[0.75rem] text-white/70">{cell}</td>)}</tr>)}</tbody></table></div></section><Faqs items={article.faqs} /></Shell>;
}

export function EnglishPageView({ path }: { path: string }) {
  const resource = englishResources.find((item) => item.path === path);
  if (resource) return <Resource article={resource} />;
